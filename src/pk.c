...
int pk_decode_double (const uint8_t *buffer, size_t blen, size_t *bseek, double *out) {
    // Historical wire format is little-endian IEEE754, unlike integer fields.
    // pk_decode_uint64 already constructs a host integer from big-endian bytes.
    ...
    for (int i = 0; i < argc; i++) {
        ...
        switch (type) {
            case DBTYPE_INTEGER: {
                ...
                break;
            case DBTYPE_FLOAT: {
                // Encode doubles as IEEE754 64-bit, little-endian.
                double value = database_value_double(argv[i]);
                if (value < 0) {value = -value; type = DATABASE_TYPE_NEGATIVE_FLOAT;}
                uint64_t bits;
                memcpy(&bits, &value, sizeof(bits));
                // Preserve the deployed little-endian double wire format on all hosts.
