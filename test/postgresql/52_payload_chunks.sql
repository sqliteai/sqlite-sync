...
-- Download spool: fill stages the whole stream once; paging it back must be
-- byte-identical to direct cloudsync_payload_chunks generation, with is_final
-- only on the last chunk and a single stable watermark.
SELECT cloudsync_payload_spool_fill('stream-A', 0, cloudsync_siteid(), false) AS spool_fill_count \gset

SELECT
  md5(string_agg(encode(payload, 'hex'), ',' ORDER BY chunk_index)) AS spool_md5,
  count(*) FILTER (WHERE is_final) AS spool_final_count,
  max(chunk_index) FILTER (WHERE is_final) AS spool_final_idx,
  max(chunk_index) AS spool_max_idx,
  count(DISTINCT watermark) AS spool_watermark_distinct
FROM cloudsync_payload_spool WHERE stream_id = 'stream-A' \gset

SELECT md5(string_agg(encode(payload, 'hex'), ',' ORDER BY chunk_index)) AS direct_md5
FROM cloudsync_payload_chunks(0, cloudsync_siteid(), NULL, false) \gset

SELECT (:spool_fill_count::int = :chunk_count::int
  AND :'spool_md5' = :'direct_md5'
  AND :spool_final_count::int = 1
  AND :spool_final_idx::int = :spool_max_idx::int
  AND :spool_watermark_distinct::int = 1) AS spool_ok \gset
\if :spool_ok
\echo [PASS] (:testid) Spool fill/page is byte-identical to direct generation (:spool_fill_count chunks, final on last)
\else
\echo [FAIL] (:testid) Spool fill/page mismatch (fill=:spool_fill_count vs :chunk_count, final_count=:spool_final_count)
SELECT (:fail::int + 1) AS fail \gset
\endif

-- Idempotent re-fill (no duplicate rows), empty window -> 0, drop reports count.
SELECT cloudsync_payload_spool_fill('stream-A', 0, cloudsync_siteid(), false) AS spool_refill_count \gset
SELECT count(*) AS spool_rows_after_refill FROM cloudsync_payload_spool WHERE stream_id = 'stream-A' \gset
SELECT cloudsync_payload_spool_fill('stream-empty', 999999, cloudsync_siteid(), false) AS spool_empty_count \gset
SELECT cloudsync_payload_spool_drop('stream-A') AS spool_drop_count \gset
SELECT count(*) AS spool_rows_after_drop FROM cloudsync_payload_spool WHERE stream_id = 'stream-A' \gset

SELECT (:spool_refill_count::int = :chunk_count::int
  AND :spool_rows_after_refill::int = :chunk_count::int
  AND :spool_empty_count::int = 0
  AND :spool_drop_count::int = :chunk_count::int
  AND :spool_rows_after_drop::int = 0) AS spool_lifecycle_ok \gset
\if :spool_lifecycle_ok
\echo [PASS] (:testid) Spool idempotent re-fill, empty window, and drop behave correctly
\else
\echo [FAIL] (:testid) Spool lifecycle mismatch (refill=:spool_refill_count empty=:spool_empty_count drop=:spool_drop_count after_drop=:spool_rows_after_drop)
SELECT (:fail::int + 1) AS fail \gset
\endif
...
VALUES ('stale', 0, '\x00'::bytea, 1, 1, 1, 1, true, extract(epoch FROM now())::bigint - 90000);
SELECT cloudsync_payload_spool_fill('stream-B', 0, cloudsync_siteid(), false) AS _spool_gc_fill \gset
SELECT
  (SELECT count(*) FROM cloudsync_payload_spool WHERE stream_id = 'stale') AS stale_remaining,
  (SELECT count(*) FROM cloudsync_payload_spool WHERE stream_id = 'stream-B') AS streamb_count \gset
SELECT (:stale_remaining::int = 0 AND :streamb_count::int = :chunk_count::int) AS spool_gc_ok \gset
\if :spool_gc_ok
\echo [PASS] (:testid) Spool stale-GC reaps abandoned streams on fill
\else
\echo [FAIL] (:testid) Spool stale-GC did not reap the abandoned stream (stale_remaining=:stale_remaining)
SELECT (:fail::int + 1) AS fail \gset
\endif
