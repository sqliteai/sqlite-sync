...
\set testid '39-chunks'
\ir helper_test_init.sql

\connect postgres
\ir helper_psql_conn_setup.sql
DROP DATABASE IF EXISTS cloudsync_test_39_chunks_src;
DROP DATABASE IF EXISTS cloudsync_test_39_chunks_dst;
DROP DATABASE IF EXISTS cloudsync_test_39_chunks_legacy;
CREATE DATABASE cloudsync_test_39_chunks_src;
CREATE DATABASE cloudsync_test_39_chunks_dst;
CREATE DATABASE cloudsync_test_39_chunks_legacy;

\connect cloudsync_test_39_chunks_src
\ir helper_psql_conn_setup.sql
...
\connect cloudsync_test_39_chunks_dst
\ir helper_psql_conn_setup.sql
CREATE EXTENSION IF NOT EXISTS cloudsync;
CREATE TABLE payload_chunk_test (
  id TEXT PRIMARY KEY,
...
FROM chunk_transport
WHERE get_byte(payload, 4) = 3
ORDER BY ord
LIMIT 1 \gset

SELECT (COUNT(*) = 0) AS stale_cleanup_ok
FROM cloudsync_payload_fragments
WHERE value_id = 'stale-incomplete' \gset

\if :stale_cleanup_ok
\echo [PASS] (:testid) Stale incomplete fragment cleanup works
\else
\echo [FAIL] (:testid) Stale incomplete fragment cleanup failed
SELECT (:fail::int + 1) AS fail \gset
\endif

\connect cloudsync_test_39_chunks_legacy
\ir helper_psql_conn_setup.sql
CREATE EXTENSION IF NOT EXISTS cloudsync;
CREATE TABLE payload_chunk_test (
  id TEXT PRIMARY KEY,
